...
def alltags(gitrepo):
    return list((remove_prefix(e, "refs/tags/"), str(gitrepo.lookup_reference_dwim(e).target))
...
def allbranches(gitrepo):
    return [(remove_prefix(e, "origin/"), str(gitrepo.lookup_reference_dwim(e).target))
        ...
        if "github" in entry or "git" in entry or "zipfiles" in entry:
            if "github" in entry:
                ...
                if entry.get("bitbucket-api-downloads-instead-of-tags"):
                    ...
                    for download in bitbucket._get_paged(
                        "2.0/repositories/%s/downloads" %
                            entry.get("bitbucket-api-downloads-instead-of-tags")):
                        name = download["name"]
                        if name.endswith(".zip") and name.startswith(key):
                            ...
                            if ver.major == 0 and ver.minor == 0 and ver.patch == 0:
                                continue
                            tags.append(
                                ("v" + str(ver), download["links"]["self"]["href"]))
                else:
            ...
            for (tagName, sha) in objects:
                ...
                for build in v.build:
                    if "-" in build:
                        raise Exception(
                            "Release build string looks like a pre-release: %s" % v)
                ...
                entrykind = "zip" if (
                    "zipfiles" in entry or sha.startswith("http")) else "sha"
                ...
                if (entrykind not in thisTag) or (thisTag[entrykind] != sha):
                    if entrykind == "zip":
                        ...
                        with open(zipfilepath, 'wb') as fout:
                            fout.write(
                                requests.get(
                                    sha, allow_redirects=True).content)
                        ...
                        try:
                            ...
                            print(
                                "Failed to checkout %s from repo: %s with SHA %s" %
                                (tagName, repopath, sha))
                            print(
                                "Usually this error happens if somebody force-pushed to a repository.")
                            print(
                                "We will delete our own cache of %s now!", repopath)
                    ...
                    for libname in names:
                        ...
                        if len(hits) == 1:
                            ...
                            for extraPath in (
                                ...
                                hits += hitsNew
                        if len(hits) != 1:
                            print(str(len(hits)) + " hits for " +
                                  libname + " in " + tagName + ": " + str(hits))
                            print(
                                "Check that " +
                                libname +
                        ...
                        if libname not in classNamesAfterLoad:
                            ...
                            continue
                        version = omc.sendExpression(
                            "getVersion(%s)" % libname)
                        if version == "":
                        ...
                        if version.startswith("+"):
                            version = version[1:]
                        uses = sorted([[e[0], str(common.VersionNumber(e[1]))]
                                      for e in omc.sendExpression("getUses(%s)" % libname)])
                        for ignore in entry.get("ignore-uses", []):
                            uses = [use for use in uses if use[0] != ignore]
                        # Get conversions
                        (withoutConversion, withConversion) = omc.sendExpression(
                            "getConversionsFromVersions(%s)" % libname)
                        withoutConversion = list(filter(None, [str(ver) for ver in sorted(
                            [common.VersionNumber(v) for v in withoutConversion])]))
                        withConversion = list(filter(None, [str(ver) for ver in sorted(
                            [common.VersionNumber(v) for v in withConversion])]))
            ...
            raise Exception(
                "Don't know how to handle entry for %s: %s" %
                (key, entry))
