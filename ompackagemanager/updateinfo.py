...
Branch = collections.namedtuple('Branch', ['name', 'sha'])
Tag = collections.namedtuple('Tag', ['name', 'sha'])


bitbucket = Bitbucket(url="https://api.bitbucket.org")


def removerepo(ghurl: str, repopath: str) -> None:
...
def alltags(gitrepo: pygit2.Repository) -> list[Tag]:
    return list(Tag(remove_prefix(e, "refs/tags/"), str(gitrepo.lookup_reference_dwim(e).target))
                for e in filter(lambda r: tagregex.match(r), gitrepo.listall_references()))
...
def allbranches(gitrepo: pygit2.Repository) -> list[Branch]:
    return [Branch(remove_prefix(e, "origin/"), str(gitrepo.lookup_reference_dwim(e).target))
            for e in gitrepo.branches.remote]


def getgitrepo(url: str, repopath: str) -> pygit2.Repository:
...
def collect_branches_tags(entry: dict, key: str, repopath: str,
    ...
    if "github" in entry:
        ...
        bitbucket_api_download = entry.get("bitbucket-api-downloads-instead-of-tags")
        if bitbucket_api_download:
            tags = []
            for download in bitbucket._get_paged("2.0/repositories/%s/downloads" % bitbucket_api_download):
                name = download["name"]
...
def new_libentry(libname: str,
    ...
    Args:
        ...
        tagName (str): Name of Git tag
        entry (_type_): Entry from repos.json
        hits (list[str]): Possible locations for Modelica file
        repopath (str): Path to git repository
        omc (OMPython.OMCSessionZMQ): OMC session

    Returns:
        dict[str, str]: `libentry` with keys `version`, `path`, `uses`, `provides` and `convertFromVersion`.
    """

    version = omc.sendExpression("getVersion(%s)" % libname)
    if not isinstance(version, str):
        version = ""
    version = normalize_version(version, tagName, entry)

    uses = omc.sendExpression("getUses(%s)" % libname)
    if uses is None:
        uses = []
    uses = sorted([[e[0], str(common.VersionNumber(e[1]))] for e in uses])
            ...
            for (tagName, sha) in objects:
                ...
                if (entrykind not in thisTag) or (thisTag[entrykind] != sha):
                    ...
                    for libname in names:
                        ...
                        libentry = new_libentry(libname, tagName, entry, hits, repopath, omc)
